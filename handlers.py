...
async def send_issue(bot, chat_id, issue, caption=None):
    ...
    try: await bot.send_photo(chat_id=chat_id, photo=issue["photo_file_id"], caption=cap)
    except Exception: await bot.send_message(chat_id=chat_id, text=cap)
...
async def edit_caption(ctx, chat_id, msg_id, updates, issue_id, old_cap):
    ...
    except Exception as e: print(f"[EDIT_CAPTION] {e}")

async def group_msg(ctx, issue_id, text):
    try: await ctx.bot.send_message(chat_id=get_group_id(issue_id), text=text)
    except Exception as e: print(f"[GROUP_MSG] {e}")

# ── Commands ──────────────────────────────────────────────────────────────────

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.chat_data.clear()
    await update.message.reply_text("👋 Welcome to Company Issue Reporter Bot\n\nPlease click below to report an issue.",
...
async def issue_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await (send_issue(context.bot, update.effective_chat.id, issue) if issue
           else update.message.reply_text(f"Issue #{context.args[0]} not found."))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in STATUS_MAP:
        ...
        try: issues = sorted(issues, key=lambda x: x["id"] if isinstance(x, dict) else x[0], reverse=True)[:20]
        except Exception: issues = issues[:20]
        buttons = [[InlineKeyboardButton(f"#{i['id'] if isinstance(i,dict) else i[0]}",
                    callback_data=f"view_issue_{i['id'] if isinstance(i,dict) else i[0]}")] for i in issues]
        if not buttons: await query.message.reply_text(f"No {status} issues found."); return
    ...
    if data.startswith("view_issue_"):
        ...
        await (send_issue(context.bot, query.message.chat.id, issue) if issue
               else query.message.reply_text("Issue not found.")); return
    ...
    if data.startswith("assign_"):
        ...
        await query.message.reply_text(f"👷 Type the technician name for Issue #{issue_id}."); return
    ...
    if data.startswith("status_"):
        ...
        if status_text in ("Rejected", "Fixed"):
            ...
            await query.message.reply_text(
                ...
                else f"✅ Please type what was fixed for Issue #{issue_id}."); return
        changed_by = query.from_user.first_name
        set_status(issue_id, status_text, changed_by)
        issue = fetch_ticket(issue_id)
        if issue: await notify_user(context, issue["requester_id"], NOTIFY_TEXT[status_text].format(id=issue_id))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text, step, cd = update.message.text.strip(), context.chat_data.get("step"), context.chat_data
    ...
    # Admin flow — reject and fix share one branch
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.send_photo(chat_id=group_id, photo=photo_file_id,
                                      caption=caption, reply_markup=status_keyboard(issue_id))
    except Exception as e: print(f"[GROUP SEND] Issue #{issue_id}: {e}")
